  ...
  return (
    <nav className="bg-[#353636] w-full sticky top-0 z-40 shadow-md">
      <div className="relative flex sm:flex-col xs:flex-col mx-auto items-center justify-between py-4 px-6 max-w-screen-xl xs:space-y-2 sm:space-y-2">
        ...
        />
        <div className="flex-1 mx-6 max-w-lg relative z-50">
          <input
            type="text"
          ...
          {suggestions.length > 0 ? (
            <ul className="absolute w-full bg-[#353636] border mt-4 border-gray-600 text-white z-50 rounded-md max-h-40 overflow-y-auto">
              {suggestions.map((suggestion, index) => (
                <li key={index} onClick={() => handleAnimeInfo(suggestion.name)} className="flex items-center z-50 px-4 py-2 hover:bg-[#3f4040] cursor-pointer">
