...
async function generateSitemap() {
  try {
    ...
    for (const anime of animes) {
      ...
      // Add anime info page
      sitemap += `<url>\n  <loc>https://www.animeelysium.com/animeInfo/name?query=${animeUrlName}</loc>\n  <changefreq>daily</changefreq>\n  <priority>0.8</priority>\n</url>\n`;

      // Add episode pages
      const episodes = await AnimeEpisode.find({ ANIME_ID: anime._id }).exec();
      for (const episode of episodes) {
        sitemap += `<url>\n  <loc>https://www.animeelysium.com/mobile-anime/name?query=${animeUrlName}&episode=${episode.EPISODE_NUMBER}</loc>\n  <changefreq>daily</changefreq>\n  <priority>0.8</priority>\n</url>\n`;
      }

      // Add category pages
      for (const category of anime.CATEGORIES) {
        const categoryUrlName = encodeURIComponent(category);
        sitemap += `<url>\n  <loc>https://www.animeelysium.com/categories/category?query=${categoryUrlName}</loc>\n  <changefreq>daily</changefreq>\n  <priority>0.8</priority>\n</url>\n`;
      }
    }
